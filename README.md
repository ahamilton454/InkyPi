...
## About InkyPi 
InkyPi is an open-source, customizable E-Ink display powered by a Raspberry Pi. It keeps setup and configuration simple, so it is easy to display the content that matters most with a straightforward web interface.
...
- Easy installation and configuration make it a great fit for beginners and makers alike
...
After the installation completes, the script prompts for a Raspberry Pi reboot. Once it restarts, the display updates to show the InkyPi splash screen.
...
Waveshare offers a range of e-Paper displays that are similar to the Inky screens from Pimoroni, with slightly different requirements. Inky displays auto configure through the inky Python library, while Waveshare displays require model specific drivers from their [Python EPD library](https://github.com/waveshareteam/e-Paper/tree/master/RaspberryPi_JetsonNano/python/lib/waveshare_epd).
...
- [rpi_weather_display](https://github.com/sjnims/rpi_weather_display) - alternative eink weather dashboard with strong power efficiency
